...
from presentacion.dtos import DtoVistaAtributo


class ModeloAtributos(QAbstractTableModel):
    """Expone una lista de ``DtoVistaAtributo`` en una tabla Qt."""

    ENCABEZADOS = ["Nombre", "Tipo", "Obligatorio", "Valor por defecto"]

    def __init__(self, atributos: list[DtoVistaAtributo] | None = None) -> None:
        super().__init__()
        self._atributos = atributos or []

    def actualizar(self, atributos: list[DtoVistaAtributo]) -> None:
    ...
    def atributo_en_fila(self, fila: int) -> DtoVistaAtributo | None:
