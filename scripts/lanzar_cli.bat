...
echo =============================================== > "%DEBUG_LOG%"
echo [INFO] Inicio lanzar_cli.bat: %DATE% %TIME% >> "%DEBUG_LOG%"
...
python -m presentacion.cli %* >> "%STDOUT_LOG%" 2>> "%STDERR_LOG%"
...
:fail_run
echo [ERROR] Fallo al ejecutar la CLI.
echo [ERROR] Fallo al ejecutar python -m presentacion.cli. Revisa %STDERR_LOG% >> "%DEBUG_LOG%"
pause
exit /b 6
