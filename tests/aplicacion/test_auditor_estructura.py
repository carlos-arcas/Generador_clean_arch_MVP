...
def test_auditor_estructura_valida_crea_informe(tmp_path: Path) -> None:
    ...
    resultado = AuditarProyectoGenerado(ejecutor).ejecutar(DtoAuditoriaEntrada(ruta_proyecto=str(tmp_path), blueprints_usados=["base_clean_arch"]))
...
def test_auditor_estructura_detecta_faltantes(tmp_path: Path) -> None:
    ...
    resultado = AuditarProyectoGenerado(ejecutor).ejecutar(DtoAuditoriaEntrada(ruta_proyecto=str(tmp_path)))

    assert resultado.valido is False
    assert any("No existe el recurso obligatorio" in error for error in resultado.errores)
